...
/*	public static final Target Select_SuggCat_Dairy = new Target("Select_SuggCat_Dairy",
	...
	public static final Target CreateExpenseCategoryDesc1 = new Target("CreateExpenseCategoryDesc1",
			"//*[@class='mm-c-customexpense__setup-text']//*[contains(text(),'Create one or more custom expense categories ')]",
			Target.XPATH);
	...
	public CategoryPage VerifyCustomListItemsDefaultCategoryLocation2(String string) {
		...
		try {
			...
			if ((LocationsPage.Category6[0].equalsIgnoreCase("Food"))
					|| (LocationsPage.Category6[0].equalsIgnoreCase("Non-Food"))) {
				flag1 = true;

			}
			if ((LocationsPage.Category7[0].equalsIgnoreCase("Food"))
					|| (LocationsPage.Category7[0].equalsIgnoreCase("Non-Food"))) {
				flag2 = true;

			}
			if ((LocationsPage.Category8[0].equalsIgnoreCase("Food"))
					|| (LocationsPage.Category8[0].equalsIgnoreCase("Non-Food"))) {
				flag3 = true;
	...
	public CategoryPage VerifySuggestedCategory(String string) {
		...
		try {
			...
			final Target Sysco_Product_Select = new Target("Product_Select",
					"(//*[@class='mm-c-product-list']//*[@class='mm-c-product-list__item mm-c-product__sysco ']//*[@class='mm-c-product-list__image'])[1]",
					Target.XPATH);

			if (getCommand().isTargetPresent(Sysco_Product_Select)) {
				final Target CategoryLine1 = new Target("CategoryLine1",
						"(//*[@class='mm-c-product-list']//*[@class='mm-c-product-list__all-prods']//*[@class='mm-c-product-list__details-wrapper']//*[@class='mm-c-product-list__location'])[1]",
						Target.XPATH);
...
*/
	...
	public CategoryPage VerifyOptionsOnSetUpFoodCost(String string) {
		...
		try {
			...
			if (getCommand().isTargetPresent(FoodAndNonFood) && getCommand().isTargetPresent(FoodAndNonFoodDesc)
					&& getCommand().isTargetPresent(SuggestedCategories)
					&& getCommand().isTargetPresent(SuggestedCategoriesDesc)
					&& getCommand().isTargetPresent(CustomCategories)
					&& getCommand().isTargetPresent(CustomCategoriesDesc)) {
	...
	public CategoryPage VerifyItemPresentUncategorizedAndAllItems(String string) {
		...
		try {
			...
			getCommand().click(SetupInventoryCustomCategoryAllItemsTab);
			getCommand().waitFor(3);
			final Target ItemSelectedFromUncatgorisedVerify = new Target("FirstItemSelectedFromUncatgorised",
					"//*[@id='mount']//*[contains(text(),'" + CategoryItemName1_1 + "')]", Target.XPATH);
			final Target ItemSelectedFromUncatgorisedVerify1 = new Target("FirstItemSelectedFromUncatgorised",
					"//*[@id='mount']//*[contains(text(),'" + CategoryItemName1_2 + "')]", Target.XPATH);
			final Target ItemSelectedFromUncatgorisedVerify2 = new Target("FirstItemSelectedFromUncatgorised",
					"//*[@id='mount']//*[contains(text(),'" + CategoryItemName1_3 + "')]", Target.XPATH);
			...
			if (getCommand().isTargetPresent(ItemSelectedFromUncatgorisedVerify)) {
				...
				log(" Added item  is present in category Tab(Expected-Item should not be present) :Fail",
						LogType.VERIFICATION_STEP);
				throw new Exception();
			} else if (!((getCommand().isTargetPresent(ItemSelectedFromUncatgorisedVerify))
					&& (getCommand().isTargetPresent(ItemSelectedFromUncatgorisedVerify1))
