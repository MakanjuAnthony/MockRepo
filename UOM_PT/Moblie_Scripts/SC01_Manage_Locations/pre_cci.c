# 1 "e:\\uom\\mobility scripts\\final\\sc01_manage_locations\\\\combined_SC01_Manage_Locations.c"
...
# 1 "e:\\uom\\mobility scripts\\final\\sc01_manage_locations\\\\combined_SC01_Manage_Locations.c" 2
...
# 2 "e:\\uom\\mobility scripts\\final\\sc01_manage_locations\\\\combined_SC01_Manage_Locations.c" 2
...
# 3 "e:\\uom\\mobility scripts\\final\\sc01_manage_locations\\\\combined_SC01_Manage_Locations.c" 2
...
# 4 "e:\\uom\\mobility scripts\\final\\sc01_manage_locations\\\\combined_SC01_Manage_Locations.c" 2
...
# 47 "Action.c"
	...
	if(strcmp(lr_eval_string("{Corr_CustomerID1}"),"")==0)
    {
		...
		lr_end_transaction("SC01_01A_Location_Create_Account", 2);
	}
	...
	web_custom_request("ExpenseCategory",
		...
		HttpRetCode = web_get_int_property(1);
		
		if (HttpRetCode == 200)
		{
			lr_end_transaction("SC01_02_Location_Click_on_ExpenseCategory",0);
		}
		else
		{
			lr_end_transaction("SC01_02_Location_Click_on_ExpenseCategory",1);
		}
	...
	if (HttpRetCode == 201)
	{
		...
		lr_end_transaction("SC01_03_Location_Add_expenseCategory",0);
	}
	else
	{
		lr_log_message("The script failed to create Expense Category with http Request Header %d", HttpRetCode);
		
		lr_end_transaction("SC01_03_Location_Add_expenseCategory",1);
	}
	...
	web_custom_request("Locations",
		...
		if (HttpRetCode == 200)
		{
			lr_end_transaction("SC01_04_Location_Click_on_Locations",0);
		}
		else
		{
			lr_end_transaction("SC01_04_Location_Click_on_Locations",1);
		}
...
for(j=1;j<=4;j++)
{
		...
		{ 	lr_end_transaction("SC01_05_Location_Create_Location",1); 	}
	
		else
		{	lr_end_transaction("SC01_05_Location_Create_Location",0); 	
			...
			web_custom_request("LocationDetails",
				...
				{ 	lr_end_transaction("SC01_06_Location_Get_Location_Details",1); 	}
				else
				{	lr_end_transaction("SC01_06_Location_Get_Location_Details",0); 	}
		}
}
...
if(atoi(lr_eval_string("{Corr_LocationIDs_U_count}"))>1)
{
	...
	lr_end_transaction("SC01_07_Location_Edit_location",2);
...
if(atoi(lr_eval_string("{Corr_LocationIDs_count}"))>1)
{
	...
	lr_end_transaction("SC01_08_Location_Delete",2);
}
	lr_think_time(TT);
...
# 5 "e:\\uom\\mobility scripts\\final\\sc01_manage_locations\\\\combined_SC01_Manage_Locations.c" 2
...
# 6 "e:\\uom\\mobility scripts\\final\\sc01_manage_locations\\\\combined_SC01_Manage_Locations.c" 2
